...
class UploadService {
	...
	_uploadIfExists(relativePath, resultPath, disposition, uploads) {
		...
		if (fs.existsSync(filePath)) {
			let uploadUri = this._resolveUrl(this.resultsUri, resultPath || relativePath);
	...
	_resolveUrl(baseUrl, relativeUrl) {
		let parsed = url.parse(baseUrl);
		parsed.pathname = path.join(parsed.pathname, relativeUrl).replace('\\', '/', 'g');
